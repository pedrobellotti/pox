...
class LearningSwitch (object):
  ...
  def flowStatsHW (self, event):
    ...
    regrasOrdenadas = sorted(event.stats, key=lambda x: x.byte_count/x.duration_sec if x.duration_sec > 0 else 0, reverse=False) 
    regrasInseridas = 0
    limite = len(regrasOrdenadas)*0.5 #------------------------
  ...
  def packetInDL(self, event, packet):
    if (event.port == 2):
      ...
      if (protocolo == 'UDP' or protocolo == 'TCP'):
        if (msg.match.tp_src is not None and msg.match.tp_dst is not None):
          ...
          if (protosrc in self.listaPortas):
            log.debug("%s: Packet in para porta %d ja atendida, ignorando." % (self.nome, protosrc))
            return
          else:
      ...
      #Envia todo o trafego para o switch HW
      log.debug("%s: Encaminhando para switch HW." % (self.nome))
